...
{
    public class BSPDungeonAlgorithm : BlockAlgorithm
    {
        public int minLeafWidth = 10;
        public int minLeafHeight = 10;
        ...
        //public bool[] Generate2(TileMapConfig _config, int width, int height, out Vector3 _startPosition, out Vector3 _endPosition)
        //{
        //    var tmp = Generate(_config, width, height, out _startPosition, out _endPosition);
        //    bool[] tmp2 = new bool[width * height];
        //    for (int i = 0; i < height; i++)
        //    {
        //        for (int j = 0; j < width; j++)
        //        {
        //            tmp2[i * width + j] = tmp[j, i];
        //        }
        //    }
        //    return tmp2;
        //}

        public override void Generate(TilemapData map, TilemapData mask, out Vector3 _startPosition, out Vector3 _endPosition)
        {
            _splitH = false;
        ...
        void BuildHallways()
        {
            ...
            for (int i = 0; i < parentTree.Count; i++)
            {
                ...
                if (_centerPointFirstChild.x == _centerPointSecondChild.x)
                {
                    ...
                    {
                        hallways.Add(new Room((int)_centerPointFirstChild.x, (int)_centerPointFirstChild.y, corridorWidth, (int)_distZ));
                    }
                    else
                    {
                        hallways.Add(new Room((int)_centerPointFirstChild.x, (int)_centerPointFirstChild.y - (int)_distZ, corridorWidth, (int)_distZ));
                    }
                }
                ...
                else
                {
                    ...
                    {
                        hallways.Add(new Room((int)_centerPointFirstChild.x, (int)_centerPointFirstChild.y, (int)_distX, corridorWidth));
                    }
                    else
                    {
                        hallways.Add(new Room((int)_centerPointFirstChild.x - (int)_distX, (int)_centerPointFirstChild.y, (int)_distX, corridorWidth));
                    }
                }
            }
            ...
            for (int i = 0; i < parentTree.Count; i++)
            {
                ...
                if (i + 1 < parentTree.Count)
                {
                    ...
                    if (Random.Range(0f, 1f) <= 0.5f)
                    {
                        ...
                        // Horizontal corridor
                        hallways.Add(new Room(_minX, _minY, _maxX - _minX, corridorWidth));
                        // Vertical corridor
                        hallways.Add(new Room(_minX, _minY, corridorWidth, _maxY - _minY));
